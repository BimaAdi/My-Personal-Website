...
export const getPaginateBlogService = async ({
	...
	page_size,
	search,
}: { page: number; page_size: number; search: string | null }): Promise<
	| Ok<PaginateBlogType>
...
> => {
	try {
		...
		return {
			status: 500,
			json: { error: err },
		} satisfies InternalServerError<InternalServerErrorType>;
	}
};
...
> => {
	try {
		...
		if (blog === null) {
			...
			} satisfies NotFound<NotFoundType>;
		}
		...
		let detail_blog = {
			...
			body: marked.parse(readBlogMd(blog.link), {
				mangle: false,
				headerIds: false,
			}),
