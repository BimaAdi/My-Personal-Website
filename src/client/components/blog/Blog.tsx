...
const Blog = () => {
	...
	return (
		<div id="blogList" className="text-white">
			...
			{paginateBlogQuery.status === "pending" ? (
				<div className="m-4">Loading...</div>
			) : (
				<></>
			)}
			{paginateBlogQuery.data?.status === 200 ? (
				paginateBlogQuery.data.json?.results.map((x) => {
					return (
						<BlogItem
							key={x.id}
							title={x.title}
			...
			{paginateBlogQuery.data?.status === 500 ? (
				<div>Something Wrong with seerver</div>
			) : (
				<></>
			)}
		</div>
